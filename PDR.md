...
它的目标不是训练模型，也不是做一个普通聊天机器人，而是把一次复杂问题回答变成一个后端流程：多条候选答案路径并行生成，经过评审、淘汰、交叉、变异、验证和预算控制后，产出一个带有演化轨迹和验证报告的结果。

当前版本定位为 **backend core**：重点是进化引擎、成本控制、验证报告、图结构数据、API/SDK/CLI，不包含正式前端。
...
- [x] 多 Thinker 候选答案生成
- [x] 锦标赛选择、精英保留、交叉重组、变异
- [x] Pairwise Elo / Swiss Tournament 排名
- [x] Red Team、Debate、Pressure Test 等后进化检查
- [x] Web Search 上下文注入，支持 Tavily / Serper
- [x] Claim extraction、verification report、claim verification report
- [x] Answer graph、lineage graph、trajectory log、trajectory replay
- [x] Cost breakdown、resource report、token budget report
- [x] 用户可切换 token budget control: `off` / `auto` / `relaxed` / `strict`
- [x] Checkpoint 保存与恢复
- [x] FastAPI、WebSocket、Python SDK、CLI
- [x] 旧前端文件已移除，当前仓库只保留后端核心和接口层
...
- [x] JSON结果持久化
- [x] YAML配置文件支持（evohive.yaml）
- [x] GitHub仓库: https://github.com/0xF69/EvoHive
